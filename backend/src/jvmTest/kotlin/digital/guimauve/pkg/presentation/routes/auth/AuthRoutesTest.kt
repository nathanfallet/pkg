...
class AuthRoutesTest {
    ...
    /**
     * Renders through the layout, so this covers the `t` directive resolving the message bundle.
     */
    @Test
    fun testLoginPage() = testApplication {
        ...
        with(response.bodyAsText()) {
            assertContains(this, "<title>Sign in — PKG</title>")
            assertContains(this, """<label for="email" class="form-label">Email address</label>""")
        }
    }
    ...
    fun testLoginWithWrongCredentials() = testApplication {
        ...
        assertContains(response.bodyAsText(), "Invalid credentials")
    }
